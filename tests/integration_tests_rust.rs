...
async fn bolt12_manual_invoice_handling() {
	...
	// Node B: receiver, with the default config.
	let config_b = random_config(true);
	let node_b = setup_node(&chain_source, config_b);
	...
	// Sleep until we broadcasted a node announcement, then a bit more to let it propagate.
	while node_b.status().latest_node_announcement_broadcast_timestamp.is_none() {
		tokio::time::sleep(std::time::Duration::from_millis(10)).await;
	}
	tokio::time::sleep(std::time::Duration::from_secs(1)).await;

	// Test 1: Manually pay a received invoice.
	let expected_amount_msat = 100_000_000;
	let offer =
	...
	// Instead of paying automatically, we should get a `Bolt12InvoiceReceived` event.
	let event = node_a.next_event_async().await;
	match event {
	...
	// Test 2: Abandon a received invoice instead of paying it.
	let offer2 =
